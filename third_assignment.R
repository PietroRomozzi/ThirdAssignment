...
## Then, we also load other data needed: the gini index calculated for the macro area considered. Again, we previously have cleaned the datas on excel.

gini <- read.csv("TidyData_gini.csv", header = T, sep =";", dec = ",", fill = T)

## Loading data for suicides (number of suicides per year).

suicides <- read.csv("TidyData_suicides.csv", header = T, sep =";", dec = ",", fill = T)
